...
suppressWarnings(resulta <- nlminb(c(1,0.1,0.001,1,100),f,lower=c(0,-Inf,0,0,0),upper=c(Inf,Inf,Inf,Inf,Inf)))
h <- function(p) { sqrt(w)*(log(m)-log(p[1]+p[2]*x+p[3]*x^2+p[4]/(p[5]-x))) }
suppressWarnings(resultb <- nls.lm(c(1,0.1,0.001,1,100),h,lower=c(0,-Inf,0,0,0),upper=c(Inf,Inf,Inf,Inf,Inf)))
